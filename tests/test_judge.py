...
class TestReadVerdict:
    ...
    def test_invalid_escape_returns_error(self, tmp_path):
        """Without JSON sanitization, invalid escapes cause parse errors."""
        p = tmp_path / "verdict.json"
    ...
    def test_invalid_unicode_escape_returns_error(self, tmp_path):
        """Without JSON sanitization, invalid unicode escapes cause parse errors."""
        p = tmp_path / "verdict.json"
...
class TestReadBatchVerdict:
    ...
    def test_invalid_escape_returns_error(self, tmp_path):
        ...
        results = _read_batch_verdict(str(p), 1)
        assert results[0]["met"] is None
        assert "invalid JSON" in results[0]["reasoning"]
